...
<%
    ...
    if (request.getParameter("id") != null && !request.getParameter("id").isEmpty() && !request.getParameter("id").trim().isEmpty()) {
        ...
        product = db.getProduct(Integer.parseInt(request.getParameter("id")));
    ...
    <tr>
        ...
        <td valign="top" class=tdNormal height="100%" width="100%" bgcolor="#FFFEF9">
            <table border="0" cellpadding="10" cellspacing="0" width="100%">
                <tr>
                    <td width="100%"><H1 class=H1><%=isEdit ? "Edit" : "Add"%> new product</H1>
                        <form method="post" action="<%= action %>">
                            <input style="display:none;" hidden type="text" name="id" size="20" value="<%=  product != null ? product.getId() : "" %>"></td>
                            <table border="0" cellpadding="3" cellspacing="0" width="100%" class=table1>
                                <tr>
                                    <td width="22%" class=tdEven>Brand</td>
                                    <td width="78%" class=tdEven>
                                        <input type="text" name="brand" size="20" value="<%=  product != null ? product.getBrand() : "" %>"></td>
                                </tr>
                                <tr>
                                    <td width="22%" class=tdodd>Made In</td>
                                    <td width="78%" class=tdOdd>
                                        <input type="text" name="madein" size="20" value="<%= product != null ? product.getMadein() : "" %>"></td>
                                </tr>
                                <tr>
                                    <td width="22%" class=tdEven>Name</td>
                                    <td width="78%" class=tdEven>
                                        <input type="text" name="name" size="20" value="<%= product != null ? product.getName() : "" %>"></td>
                                </tr>
                                <tr>
                                    <td width="22%" class=tdOdd>Price</td>
                                    <td width="78%" class=tdOdd>
                                        <input type="text" name="price" size="20" value="<%= product != null ? product.getPrice() : "" %>"></td>
                                </tr>
                                <tr>
